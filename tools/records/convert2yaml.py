...
Renamed_Records = {
    ...
    'proxy.config.output.logfile': 'proxy.config.output.logfile.name', 'proxy.config.exec_thread.autoconfig': 'proxy.config.exec_thread.autoconfig.enabled', 'proxy.config.hostdb': 'proxy.config.hostdb.enabled', 'proxy.config.tunnel.prewarm': 'proxy.config.tunnel.prewarm.enabled', 'proxy.config.ssl.origin_session_cache': 'proxy.config.ssl.origin_session_cache.enabled', 'proxy.config.ssl.session_cache': 'proxy.config.ssl.session_cache.value', 'proxy.config.ssl.TLSv1_3': 'proxy.config.ssl.TLSv1_3.enabled', 'proxy.config.ssl.client.TLSv1_3': 'proxy.config.ssl.client.TLSv1_3.enabled'
}
    ...
    for l in fileinput.input(files=file):
        ...
        if len(elements) == 4:
            ...
            if rec_name in Renamed_Records:
                ...
                continue
        # if elements len diff from 4 .. well.. your problem.
        temp.write(l.encode('utf-8'))  # no need to rename it.
...
def handle_file_input(args):
    ...
    # we will first rename the renamed fieldnames.
    config = {}
