...
public:
  ...
  struct Context {
     ...
     * which will later by used to construct the tunnel destination, this will only happen
     * if `tunnel_route` was set with a matching group.
     */
...
private:
   ...
   * captured group from the `fqdn`, this function will replace them by the proper group string.
   */
  std::string
  replace_match_groups(std::string dst, const std::vector<std::string> &groups) const
  {
    ...
    for (auto c = std::begin(dst); c != end; c++, pos++) {
      if (*c == '$') {
        ...
        if (!is_number(number_str)) {
          // it may be some issue on the configured string, append the char and keep going.
          real_dst += *c;
          continue;
        }
