...
TSPluginInit(int argc, const char *argv[])
{
  ...
  auto register_method = [&](std::string const &rpcCallName, TSRPCMethodCb func) {
    if (TSRPCRegisterMethodHandler(rpcCallName.c_str(), func, rpcRegistrationInfo) == TS_ERROR) {
  ...
  register_method("test_join_hosts_method", test_join_hosts_method);
  register_method("test_io_on_et_task", test_io_on_et_task);

  // Register join host notification.
  if (TSRPCRegisterNotificationHandler("test_join_hosts_notification", test_join_hosts_notification, rpcRegistrationInfo) ==
      TS_ERROR) {
