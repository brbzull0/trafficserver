...
**Optional.** Inline configuration content specified as a YAML block
scalar (``|``).  The literal text is written to a temporary file at
startup and the path is passed to the plugin as an argument, so existing
plugins work without modification.

Use a block scalar to preserve the exact text -- this is important for
plugins like ``txn_box.so`` that assign special meaning to YAML quoting.
...
(e.g. ``<config_dir>/.header_rewrite_inline_1.conf``).
