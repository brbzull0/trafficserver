...
void YAMLNodeToRecord(const CfgNode &field, swoc::Errata &errata);
...
swoc::Errata RecYAMLConfigFileParse(const char *path, YAMLNodeHandler handler);

///
/// @brief This function parses the YAML root node ("ts") and convert each field
///        into a record style object.
///
/// This function reads up from the records root node (ts) and then it flatten each yaml field
/// to match the internal record name style. i.e.:
///
///   exec_thread:
///       autoconfig:
///         scale: 1.0
...
swoc::Errata ParseRecordsFromYAML(YAML::Node root, YAMLNodeHandler handler, bool lock = false);
