...
template <typename Func>
inline bool
add_method_handler_from_plugin(const std::string &name, Func &&call, const RPCRegistryInfo *info = nullptr)
{
  return JsonRPCManager::instance().add_method_handler_from_plugin(name, std::forward<Func>(call), info);
}
