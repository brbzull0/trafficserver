...
//
void
FileManager::addFile(const char *fileName, const char *configName, bool root_access_needed, bool shouldExist,
                     ConfigManager *parentConfig)
{
  ink_mutex_acquire(&accessLock);
  addFileHelper(fileName, configName, root_access_needed, shouldExist, parentConfig);
  ink_mutex_release(&accessLock);
}

// caller must hold the lock
void
FileManager::addFileHelper(const char *fileName, const char *configName, bool root_access_needed, bool shouldExist, ConfigManager *parentConfig)
{
  ink_assert(fileName != nullptr);

  ConfigManager *rb = new ConfigManager(fileName, configName, root_access_needed, shouldExist, parentConfig);
  rb->configFiles   = this;
...
FileManager::configFileChild(const char *parent, const char *child)
{
  ...
  if (auto it = bindings.find(parent); it != bindings.end()) {
    ...
    addFileHelper(child, "", parentConfig->rootAccessNeeded(), parentConfig->fileShouldExist(), parentConfig);
  }
  ink_mutex_release(&accessLock);
}
