...
class RPC
{
  using NotificationList = std::vector<NotificationHandler>;

private:
  // We use the yamlcpp library by default.
  using Decoder = yamlcpp_json_decoder;
  using Encoder = yamlcpp_json_encoder;

protected: // protected: For unit test.
  RPC()            = default;
  RPC(RPC const &) = delete;
  RPC(RPC &&)      = delete;
  RPC &operator=(RPC const &) = delete;
...
private:
  ...
  class Distpatcher
  {
    using response_type = std::pair<std::optional<RpcResponseInfo>, std::error_code>;
  ...
  public:
    ...
    response_type distpatch(RpcRequestInfo const &request) const;
    ...
    std::pair<MethodHandler, bool> find_handler(const std::string &methodName) const;

    /// @brief  Find a particular registered handler(notification) by its associated name.
    /// @return A pair. The handler itself and a boolean flag indicating that the handler was found. If not found, second will
    ///         be false and the handler null.
    std::pair<NotificationHandler, bool> find_notification_handler(const std::string &notificationName) const;
    ...
    void show_registered_handlers(std::string_view const &, const YAML::Node &, RpcHandlerResponse &resp);

  private:
    response_type invoke_handler(RpcRequestInfo const &request) const;
    void invoke_notification_handler(RpcRequestInfo const &notification, std::error_code &ec) const;
...
private:
  ...
  Distpatcher _distpather;
};

/// Template impl.
template <typename Handler>
bool
RPC::add_handler(const std::string &name, Handler &&call)
{
  return _distpather.add_handler(name, std::forward<Handler>(call));
}

template <typename Handler>
bool
RPC::add_notification_handler(const std::string &name, Handler &&call)
{
  return _distpather.add_notification_handler(name, std::forward<Handler>(call));
}

template <typename Handler>
bool
RPC::Distpatcher::add_handler(std::string const &name, Handler &&handler)
{
...
template <typename Handler>
bool
RPC::Distpatcher::add_notification_handler(std::string const &name, Handler &&handler)
{
  std::lock_guard<std::mutex> lock(_mutex);
