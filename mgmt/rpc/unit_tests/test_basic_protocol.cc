...
struct JsonRpcUnitTest : jsonrpc::RPC {
  JsonRpcUnitTest() : RPC() {}
};

enum class TestErrors { ERR1 = 9999, ERR2 };
inline void
test_callback_ok_or_error(std::string_view const &id, YAML::Node const &params, jsonrpc::RpcHandlerResponse &resp)
{
  ...
  SECTION("Registring the method")
  {
    ...
    SECTION("Calling the method")
    {
      ...
      const std::string_view expected =
        R"({"jsonrpc": "2.0", "error": {"code": 9, "message": "Error during execution", "data": [{"code": 0, "message": "Just an error message to add more meaning to the failure"}]}, "id": "14"})";
      REQUIRE(*json == expected);
    }
  }
}
...
TEST_CASE("Basic test, batch calls", "[methods][notifications]")
{
  ...
  SECTION("inserting multiple functions, mixed method and notifications.")
  {
    ...
    SECTION("we call in batch, two functions and one notification")
    {
      ...
      REQUIRE(resp1);
      const std::string_view expected =
        R"([{"jsonrpc": "2.0", "result": {"ran": "ok"}, "id": "13"}, {"jsonrpc": "2.0", "error": {"code": 9, "message": "Error during execution", "data": [{"code": 0, "message": "Just an error message to add more meaning to the failure"}]}, "id": "14"}])";
...
TEST_CASE("Basic test with member functions(add, remove)", "[basic][member_functions]")
{
  struct TestMemberFunctionCall {
    ...
    {
      return rpc.add_handler("member_function", jsonrpc::method_handler_from_member_function(&TestMemberFunctionCall::test, this));
    }
    void
    test(std::string_view const &id, const YAML::Node &req, jsonrpc::RpcHandlerResponse &resp)
    {
      resp.result = "grand!";
    }
...
subtract(std::string_view const &id, YAML::Node const &params, jsonrpc::RpcHandlerResponse &res)
{
  const YAML::Node &numbers = params;
...
[[maybe_unused]] static void
sum(std::string_view const &id, YAML::Node const &params, jsonrpc::RpcHandlerResponse &res)
{
  int total{0};
  for (auto n : params) {
    total += n.as<int>();
...
[[maybe_unused]] static void
get_data(std::string_view const &id, YAML::Node const &params, jsonrpc::RpcHandlerResponse &res)
{
