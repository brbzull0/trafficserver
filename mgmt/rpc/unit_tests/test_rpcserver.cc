...
#include "test_utils/LocalSocketClient.h"
...
DEFINE_JSONRPC_PROTO_FUNCTION(some_foo) // id, params, resp
{
  int dur{1};
  try {
...
TEST_CASE("Sending 'concurrent' requests to the rpc server.", "[thread]")
{
  ...
  {
    jsonrpc::RPC::instance().add_handler("some_foo", &some_foo);
    jsonrpc::RPC::instance().add_handler("some_foo2", &some_foo);
...
{
  REQUIRE(jsonrpc::RPC::instance().add_handler("do_nothing", &do_nothing));
  ...
  REQUIRE(jsonrpc::RPC::instance().remove_handler("do_nothing"));
}
