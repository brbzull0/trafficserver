...
LocalUnixSocket::configure(YAML::Node const &params)
{
  try {
    ...
    // if any error, we keep  the default values? should we?
    return ts::Errata::Message{1, {ex.what()}};
  }
...
LocalUnixSocket::init()
{
  ...
  strcpy(_serverAddr.sun_path, _conf.pathName.c_str());
  ...
  while (_running) {
    ...
    if (auto fd = this->accept(ec); !ec) {
      ...
      if (auto ret = client.read_all(bw); ret) {
        ...
        if (auto response = jsonrpc::RPC::instance().handle_call(json); response) {
...
{
  int lock_fd = open(_conf.lockfileName.c_str(), O_RDONLY | O_CREAT, 0600);
  if (lock_fd == -1) {
    ec = std::make_error_code(static_cast<std::errc>(errno));
    return;
  }
...
template <> struct convert<rpc::transport::LocalUnixSocket::Config> {
  using this_type = rpc::transport::LocalUnixSocket::Config;

  static bool
  decode(const Node &node, this_type &rhs)
  {
    // it will throw on any error.
    // TODO: fix this, if first one fails, then all fails.
    if (auto n = node[this_type::PATH_NAME_KEY_STR]) {
      rhs.lockfileName = n.as<std::string>();
    }
    if (auto n = node[this_type::LOCK_FILE_KEY_STR]) {
      rhs.pathName = n.as<std::string>();
    }
    if (auto n = node[this_type::BACKLOG_KEY_STR]) {
      rhs.backlog = n.as<int>();
    }
    if (auto n = node[this_type::MAX_RETRY_ON_TR_ERROR_KEY_STR]) {
