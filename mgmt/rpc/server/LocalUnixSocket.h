...
public:
  ...
  virtual ~LocalUnixSocket() override { unlink(_conf.pathName.c_str()); }
...
private:
  struct Config {
    static constexpr auto PATH_NAME_KEY_STR{"lockfile_path_name"};
    static constexpr auto LOCK_FILE_KEY_STR{"path_name"};
    static constexpr auto BACKLOG_KEY_STR{"path_name"};
    static constexpr auto MAX_RETRY_ON_TR_ERROR_KEY_STR{"path_name"};

    std::string pathName{"/tmp/jsonrpc20.sock"};
    std::string lockfileName{"/tmp/jsonrpc20.lock"};
    int backlog{5};
