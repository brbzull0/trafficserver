...
ConfigManager::ConfigManager(const char *fileName_, const char *configName_, bool root_access_needed_, bool shouldExist_, ConfigManager *parentConfig_)
  : root_access_needed(root_access_needed_),  shouldExist(shouldExist_), parentConfig(parentConfig_)
{
  ...
  if (statFile(&fileInfo) < 0) {
    mgmt_log("[ConfigManager::ConfigManager] %s failed to load: %s", fileName, strerror(errno));
