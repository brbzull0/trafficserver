...
scalar_node_handler(const TSYAMLRecCfgFieldData *cfg, void *data)
{
  ...
  // Let's try to get the type. Non registered fields should have the type(!!int, !!float) specified in each field
  auto [found, field_type] = try_deduce_type(value);
  if (found && (expected_type != field_type)) {
    TSError("%s", swoc::bwprint(text, "[{}]'{}' field type mismatch, expected {}, got {}", PLUGIN_NAME, cfg->field_name,
  ...
  // If the type could not be deduced, then use the expected one.
  try {
    switch (field_type) {
    case TS_RECORDDATATYPE_INT:
